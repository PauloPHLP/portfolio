...
function Navbar() {
	...
	const handleNavbarOnScroll = () => {
		window.addEventListener('scroll', () => {
			...
			if (scroll > height * 0.2 && scroll < height && window.innerWidth > 768) {
				navbar.style.webkitTransition = 'opacity 0.5s ease-in-out';
				navbar.style.opacity = 0;
			} else {
				if (scroll > height) setMobileButtons('down');

				if (scroll < height) setMobileButtons('up');
	...
	return (
		<Suspense fallback="loading">
            ...
            <a id="anchor-his-works" className="disabled-section" href="#his-works">{t("navbar_work")}</a>
					...
					<li>
						<a
							id="anchor-his-testimonials"
							className="disabled-section"
							href="#his-testimonials"
						>
					...
					<li>
						...
						>
							<img
								src={united_states}
								className={
									getCurrentLanguage() === 'enUS' ? 'flag-activated' : 'flag'
								}
					...
					<li>
						<Tooltip
							...
							aria-label="Traduzir para o português."
							classes={{ tooltip: classes.customWidth }}
						>
							<img
								src={brazil_flag}
								className={
									getCurrentLanguage() === 'ptBR' ? 'flag-activated' : 'flag'
								}
